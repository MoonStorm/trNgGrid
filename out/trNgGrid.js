                ...
                compile: function (templateElement, tAttrs) {
                    ...
                    return {
                        ...
                        pre: function (scope, instanceElement, tAttrs) {
                            scope.gridOptions = scope.$eval(tAttrs[tableDirective]);
                            scope.columnOptions = new Array();
                            if (!scope.toggleSorting) {
                                scope.toggleSorting = function (modelBinding) {
            ...
            return {
                ...
                compile: function (templateElement, tAttrs) {
                    return {
                        ...
                        pre: function (scope, instanceElement, tAttrs) {
                            if (instanceElement.children("td").length == 0) {
                                if (scope.gridOptions.model && scope.gridOptions.model.length > 0) {
                                    for (var propName in scope.gridOptions.model[0]) {
                                        ...
                                        var headerCellElement = $("<td>").attr(columnDirectiveAttribute, angular.toJson(columnDefinition, false)).appendTo(instanceElement);
                    ...
                    return {
                        ...
                        pre: function (scope, instanceElement, tAttrs, controller, transclude) {
                            ...
                            if (isHeaderColumn) {
                                ...
                                var columnIndex = instanceElement.parent().children("td").index(instanceElement);
                                if (columnIndex < 0)
                                    return;
                ...
                compile: function (templateElement, tAttrs) {
                    ...
                    return {
                        post: function (scope, compiledInstanceElement, tAttrs, controller, transcludeFn) {
                            ...
                            // });
                            templateElement.attr("ng-repeat", "item in gridOptions.model | orderBy:gridOptions.orderBy:gridOptions.orderByReverse");
